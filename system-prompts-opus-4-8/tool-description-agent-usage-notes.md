...
- To resume an agent, use ${SEND_MESSAGE_TOOL_NAME} with its ID or name as the \`to\` field — that restores full context. A new ${AGENT_TOOL_NAME} call starts fresh with no memory of prior runs${CAN_FORK_CONTEXT?' (except subagent_type: "fork")':""}, so make the prompt self-contained.
- Tell the agent whether to write code or just research (search, reads, fetches), since a fresh agent doesn't see the user's intent.
