...
Before saving, update an existing file that already covers it rather than duplicating; delete memories that turn out wrong. The bar for saving: the situation will recur across sessions, or a future session acts differently for knowing it. Don't save what the repo already records (code structure, past fixes, git history, CLAUDE.md), what only matters to this conversation, or solved one-time problems — a fix that's applied and persists (install, config, code) is its own record.
